...
4. 瞬间建造：仅对建筑生效。因为兵营和战车工厂造太快不会按照实际数量出兵，所以兵营和战车工厂仅可使用 *快速建造* 。如果战车工厂仍然赶不上就多造几个战车工厂。
5. 无限超武：无限施放超级武器和伞兵之类，但是对上面的一次性 *核弹攻击* 功能无效。如果有发射井，*核弹攻击* 也是无效的。
6. 永久雷达：无视电力和是否有雷达建筑，都开启雷达。需要手动刷新(比如展开基地车)。
5. 极速攻击：攻击速度最大化。
...
12. 任务调速：开启后滑块使能，然后可以拖动滑块自定义任务速度。遭遇战模式也有效。
13. 无限电力：顾名思义。
...
22. 全科技：开启全科技。科技树刷新后请关闭该功能，仅保留 *禁止建造选项自动刷新* ，不然每次建造，选项卡都会乱跳。
...
~~注意：4倍速攻击 和 自动攻击 目前还不稳定，可能导致游戏卡死，谨慎使用！~~
使用过程中若出现~~除上述注意中描述的卡死外的~~任何Bug,请留言……o(╯□╰)o
...
`RATrainer.exe`和`aa_engine.dll`必须放在同一目录下。

编译模式必须选择`Release`。`Debug`模式会添加额外的代码导致寻址问题。

编译环境：Win7 x86_64; VS2017 Community 15.9.21
...
│  tree.txt
│  
├─aa_engine v1.20 beta 13		// aa_engine下载目录
│      aa_engine(无壳版).dll
│      aa_engine.dll
│      AA引擎说明.txt
│      烈火网下载说明.txt
│      烈火网下载频道.url
│      
├─CEScriptProcesser				// 处理汇编文本的`Python`脚本
│      process.exe
│      process.py
│      script.txt
│      
